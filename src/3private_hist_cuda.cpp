...
{
    __shared__ int s_hist[warp_num][hist_bins];
    int base;
    int pos;
    ...
    base = threadIdx.x%32;
    for (pos=0; pos<warp_num; pos++)
        s_hist[row][base*warp_num+pos] = 0;
    atomicAdd (&(s_hist[row][pixel_val]), 1);
    __syncthreads();
